...
const joi = require("../validation/savingsAccount");
...
module.exports = {
      ...
      try {
        if (auth.authorization(true, context)) {
          ...
          if (validation.error) {
            ...
            const inputObj = {
              ...validation.value,
              requiredAmount: parseInt(validation.value.requiredAmount, 10),
              createdAt: datetime,
              updatedAt: datetime,
            };
            return new SavingsAccount(inputObj).save((error) => {
              if (error) {
        ...
        if (auth.authorization(true, context)) {
          if (input._id) {
            ...
            if (validation.error) {
              ...
              const inputObj = {
                ...rest,
                requiredAmount: parseInt(rest.requiredAmount, 10),
                updatedAt: datetime,
              };
  ...
  removedById(_id, context) {
    return new Promise((resolve) => {
      try {
        if (auth.authorization(true, context)) {
          return SavingsAccount.findOneAndDelete(
            { _id: mongoose.Types.ObjectId(_id) },
            (error) => {
              if (error)
                return resolve({
  ...
  list(input, context) {
    return new Promise(async (resolve) => {
      if (auth.authorization(true, context)) {
        ...
        const { total } = await commonFunc.totalItems(match, SavingsAccount);
        const { response } = await commonFunc.queryExecutor(
